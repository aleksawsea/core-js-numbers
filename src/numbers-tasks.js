...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const distanceOX = x2 - x1;
  const distanceOY = y2 - y1;
  const distanceSquared = distanceOX ** 2 + distanceOY ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorModulus1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorModulus2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const angleCosine = scalarProduct / (vectorModulus1 * vectorModulus2);
